...
- ⚡ **原生 AsyncIO 毫秒唤醒**：指令拉取升级为纯协程事件驱动（`asyncio.Event`），零 AnyIO 线程池占用，彻底杜绝高并发长轮询下的线程耗尽；告警超过 30s 用户未干预由服务端统一自动决策兜底，消除死锁。
- 🛡️ **端到端 MCP 鉴权防护**：公网 `/mcp` 与 REST API 端点全量覆盖 Bearer API Token / X-API-Token 鉴权中间件，采用恒定时间比对（Constant-Time Compare）防范时序探测，严防公网越权。
- 📦 **客户端离线缓冲与断网自愈**：GPU 客户端集成内存 FIFO 环形缓冲与指数退避重试，公网抖动或控制面短暂不可达时自动离线暂存里程碑指标，网络恢复后无损自动补发，绝不导致训练任务崩溃。
